/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LootTemplateWhereInput } from "./LootTemplateWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { LootTemplateOrderByInput } from "./LootTemplateOrderByInput";

@ArgsType()
class LootTemplateFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => LootTemplateWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => LootTemplateWhereInput, { nullable: true })
  @Type(() => LootTemplateWhereInput)
  where?: LootTemplateWhereInput;

  @ApiProperty({
    required: false,
    type: [LootTemplateOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [LootTemplateOrderByInput], { nullable: true })
  @Type(() => LootTemplateOrderByInput)
  orderBy?: Array<LootTemplateOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { LootTemplateFindManyArgs as LootTemplateFindManyArgs };
