/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { GameEventConditionWhereUniqueInput } from "./GameEventConditionWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { GameEventConditionUpdateInput } from "./GameEventConditionUpdateInput";

@ArgsType()
class UpdateGameEventConditionArgs {
  @ApiProperty({
    required: true,
    type: () => GameEventConditionWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => GameEventConditionWhereUniqueInput)
  @Field(() => GameEventConditionWhereUniqueInput, { nullable: false })
  where!: GameEventConditionWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => GameEventConditionUpdateInput,
  })
  @ValidateNested()
  @Type(() => GameEventConditionUpdateInput)
  @Field(() => GameEventConditionUpdateInput, { nullable: false })
  data!: GameEventConditionUpdateInput;
}

export { UpdateGameEventConditionArgs as UpdateGameEventConditionArgs };
