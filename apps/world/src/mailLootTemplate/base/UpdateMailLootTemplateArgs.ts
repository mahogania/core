/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MailLootTemplateWhereUniqueInput } from "./MailLootTemplateWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { MailLootTemplateUpdateInput } from "./MailLootTemplateUpdateInput";

@ArgsType()
class UpdateMailLootTemplateArgs {
  @ApiProperty({
    required: true,
    type: () => MailLootTemplateWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => MailLootTemplateWhereUniqueInput)
  @Field(() => MailLootTemplateWhereUniqueInput, { nullable: false })
  where!: MailLootTemplateWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => MailLootTemplateUpdateInput,
  })
  @ValidateNested()
  @Type(() => MailLootTemplateUpdateInput)
  @Field(() => MailLootTemplateUpdateInput, { nullable: false })
  data!: MailLootTemplateUpdateInput;
}

export { UpdateMailLootTemplateArgs as UpdateMailLootTemplateArgs };
