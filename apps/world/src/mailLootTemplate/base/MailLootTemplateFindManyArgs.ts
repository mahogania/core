/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MailLootTemplateWhereInput } from "./MailLootTemplateWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { MailLootTemplateOrderByInput } from "./MailLootTemplateOrderByInput";

@ArgsType()
class MailLootTemplateFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => MailLootTemplateWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => MailLootTemplateWhereInput, { nullable: true })
  @Type(() => MailLootTemplateWhereInput)
  where?: MailLootTemplateWhereInput;

  @ApiProperty({
    required: false,
    type: [MailLootTemplateOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [MailLootTemplateOrderByInput], { nullable: true })
  @Type(() => MailLootTemplateOrderByInput)
  orderBy?: Array<MailLootTemplateOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { MailLootTemplateFindManyArgs as MailLootTemplateFindManyArgs };
