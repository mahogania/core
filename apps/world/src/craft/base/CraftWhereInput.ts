/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CraftLevelStatListRelationFilter } from "../../craftLevelStat/base/CraftLevelStatListRelationFilter";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";
import { StringFilter } from "../../util/StringFilter";
import { PlayerCraftListRelationFilter } from "../../playerCraft/base/PlayerCraftListRelationFilter";

@InputType()
class CraftWhereInput {
  @ApiProperty({
    required: false,
    type: () => CraftLevelStatListRelationFilter,
  })
  @ValidateNested()
  @Type(() => CraftLevelStatListRelationFilter)
  @IsOptional()
  @Field(() => CraftLevelStatListRelationFilter, {
    nullable: true,
  })
  craftLevelStats?: CraftLevelStatListRelationFilter;

  @ApiProperty({
    required: false,
    type: StringFilter,
  })
  @Type(() => StringFilter)
  @IsOptional()
  @Field(() => StringFilter, {
    nullable: true,
  })
  id?: StringFilter;

  @ApiProperty({
    required: false,
    type: () => PlayerCraftListRelationFilter,
  })
  @ValidateNested()
  @Type(() => PlayerCraftListRelationFilter)
  @IsOptional()
  @Field(() => PlayerCraftListRelationFilter, {
    nullable: true,
  })
  playerCrafts?: PlayerCraftListRelationFilter;
}

export { CraftWhereInput as CraftWhereInput };
