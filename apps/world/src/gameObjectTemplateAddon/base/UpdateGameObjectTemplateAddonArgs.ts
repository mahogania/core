/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { GameObjectTemplateAddonWhereUniqueInput } from "./GameObjectTemplateAddonWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { GameObjectTemplateAddonUpdateInput } from "./GameObjectTemplateAddonUpdateInput";

@ArgsType()
class UpdateGameObjectTemplateAddonArgs {
  @ApiProperty({
    required: true,
    type: () => GameObjectTemplateAddonWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => GameObjectTemplateAddonWhereUniqueInput)
  @Field(() => GameObjectTemplateAddonWhereUniqueInput, { nullable: false })
  where!: GameObjectTemplateAddonWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => GameObjectTemplateAddonUpdateInput,
  })
  @ValidateNested()
  @Type(() => GameObjectTemplateAddonUpdateInput)
  @Field(() => GameObjectTemplateAddonUpdateInput, { nullable: false })
  data!: GameObjectTemplateAddonUpdateInput;
}

export { UpdateGameObjectTemplateAddonArgs as UpdateGameObjectTemplateAddonArgs };
