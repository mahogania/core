/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AchievementScriptsWhereInput } from "./AchievementScriptsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AchievementScriptsOrderByInput } from "./AchievementScriptsOrderByInput";

@ArgsType()
class AchievementScriptsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AchievementScriptsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AchievementScriptsWhereInput, { nullable: true })
  @Type(() => AchievementScriptsWhereInput)
  where?: AchievementScriptsWhereInput;

  @ApiProperty({
    required: false,
    type: [AchievementScriptsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AchievementScriptsOrderByInput], { nullable: true })
  @Type(() => AchievementScriptsOrderByInput)
  orderBy?: Array<AchievementScriptsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AchievementScriptsFindManyArgs as AchievementScriptsFindManyArgs };
