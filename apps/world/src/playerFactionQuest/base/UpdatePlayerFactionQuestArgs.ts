/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PlayerFactionQuestWhereUniqueInput } from "./PlayerFactionQuestWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PlayerFactionQuestUpdateInput } from "./PlayerFactionQuestUpdateInput";

@ArgsType()
class UpdatePlayerFactionQuestArgs {
  @ApiProperty({
    required: true,
    type: () => PlayerFactionQuestWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PlayerFactionQuestWhereUniqueInput)
  @Field(() => PlayerFactionQuestWhereUniqueInput, { nullable: false })
  where!: PlayerFactionQuestWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PlayerFactionQuestUpdateInput,
  })
  @ValidateNested()
  @Type(() => PlayerFactionQuestUpdateInput)
  @Field(() => PlayerFactionQuestUpdateInput, { nullable: false })
  data!: PlayerFactionQuestUpdateInput;
}

export { UpdatePlayerFactionQuestArgs as UpdatePlayerFactionQuestArgs };
