/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AchievementBehaviourWhereUniqueInput } from "./AchievementBehaviourWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AchievementBehaviourUpdateInput } from "./AchievementBehaviourUpdateInput";

@ArgsType()
class UpdateAchievementBehaviourArgs {
  @ApiProperty({
    required: true,
    type: () => AchievementBehaviourWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AchievementBehaviourWhereUniqueInput)
  @Field(() => AchievementBehaviourWhereUniqueInput, { nullable: false })
  where!: AchievementBehaviourWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AchievementBehaviourUpdateInput,
  })
  @ValidateNested()
  @Type(() => AchievementBehaviourUpdateInput)
  @Field(() => AchievementBehaviourUpdateInput, { nullable: false })
  data!: AchievementBehaviourUpdateInput;
}

export { UpdateAchievementBehaviourArgs as UpdateAchievementBehaviourArgs };
