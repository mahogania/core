/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DocTypeStateWhereInput } from "./DocTypeStateWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DocTypeStateOrderByInput } from "./DocTypeStateOrderByInput";

@ArgsType()
class DocTypeStateFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DocTypeStateWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DocTypeStateWhereInput, { nullable: true })
  @Type(() => DocTypeStateWhereInput)
  where?: DocTypeStateWhereInput;

  @ApiProperty({
    required: false,
    type: [DocTypeStateOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DocTypeStateOrderByInput], { nullable: true })
  @Type(() => DocTypeStateOrderByInput)
  orderBy?: Array<DocTypeStateOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DocTypeStateFindManyArgs as DocTypeStateFindManyArgs };
