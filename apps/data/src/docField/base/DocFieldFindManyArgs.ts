/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DocFieldWhereInput } from "./DocFieldWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DocFieldOrderByInput } from "./DocFieldOrderByInput";

@ArgsType()
class DocFieldFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DocFieldWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DocFieldWhereInput, { nullable: true })
  @Type(() => DocFieldWhereInput)
  where?: DocFieldWhereInput;

  @ApiProperty({
    required: false,
    type: [DocFieldOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DocFieldOrderByInput], { nullable: true })
  @Type(() => DocFieldOrderByInput)
  orderBy?: Array<DocFieldOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DocFieldFindManyArgs as DocFieldFindManyArgs };
