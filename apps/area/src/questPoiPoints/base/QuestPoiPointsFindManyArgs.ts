/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestPoiPointsWhereInput } from "./QuestPoiPointsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { QuestPoiPointsOrderByInput } from "./QuestPoiPointsOrderByInput";

@ArgsType()
class QuestPoiPointsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => QuestPoiPointsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => QuestPoiPointsWhereInput, { nullable: true })
  @Type(() => QuestPoiPointsWhereInput)
  where?: QuestPoiPointsWhereInput;

  @ApiProperty({
    required: false,
    type: [QuestPoiPointsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [QuestPoiPointsOrderByInput], { nullable: true })
  @Type(() => QuestPoiPointsOrderByInput)
  orderBy?: Array<QuestPoiPointsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { QuestPoiPointsFindManyArgs as QuestPoiPointsFindManyArgs };
