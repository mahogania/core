/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestRewardChoiceItemsWhereInput } from "./QuestRewardChoiceItemsWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class QuestRewardChoiceItemsListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => QuestRewardChoiceItemsWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRewardChoiceItemsWhereInput)
  @IsOptional()
  @Field(() => QuestRewardChoiceItemsWhereInput, {
    nullable: true,
  })
  every?: QuestRewardChoiceItemsWhereInput;

  @ApiProperty({
    required: false,
    type: () => QuestRewardChoiceItemsWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRewardChoiceItemsWhereInput)
  @IsOptional()
  @Field(() => QuestRewardChoiceItemsWhereInput, {
    nullable: true,
  })
  some?: QuestRewardChoiceItemsWhereInput;

  @ApiProperty({
    required: false,
    type: () => QuestRewardChoiceItemsWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRewardChoiceItemsWhereInput)
  @IsOptional()
  @Field(() => QuestRewardChoiceItemsWhereInput, {
    nullable: true,
  })
  none?: QuestRewardChoiceItemsWhereInput;
}
export { QuestRewardChoiceItemsListRelationFilter as QuestRewardChoiceItemsListRelationFilter };
