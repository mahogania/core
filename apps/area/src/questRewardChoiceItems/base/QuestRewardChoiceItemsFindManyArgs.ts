/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestRewardChoiceItemsWhereInput } from "./QuestRewardChoiceItemsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { QuestRewardChoiceItemsOrderByInput } from "./QuestRewardChoiceItemsOrderByInput";

@ArgsType()
class QuestRewardChoiceItemsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => QuestRewardChoiceItemsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => QuestRewardChoiceItemsWhereInput, { nullable: true })
  @Type(() => QuestRewardChoiceItemsWhereInput)
  where?: QuestRewardChoiceItemsWhereInput;

  @ApiProperty({
    required: false,
    type: [QuestRewardChoiceItemsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [QuestRewardChoiceItemsOrderByInput], { nullable: true })
  @Type(() => QuestRewardChoiceItemsOrderByInput)
  orderBy?: Array<QuestRewardChoiceItemsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { QuestRewardChoiceItemsFindManyArgs as QuestRewardChoiceItemsFindManyArgs };
