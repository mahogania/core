/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LeadSourceWhereUniqueInput } from "./LeadSourceWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { LeadSourceUpdateInput } from "./LeadSourceUpdateInput";

@ArgsType()
class UpdateLeadSourceArgs {
  @ApiProperty({
    required: true,
    type: () => LeadSourceWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => LeadSourceWhereUniqueInput)
  @Field(() => LeadSourceWhereUniqueInput, { nullable: false })
  where!: LeadSourceWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => LeadSourceUpdateInput,
  })
  @ValidateNested()
  @Type(() => LeadSourceUpdateInput)
  @Field(() => LeadSourceUpdateInput, { nullable: false })
  data!: LeadSourceUpdateInput;
}

export { UpdateLeadSourceArgs as UpdateLeadSourceArgs };
