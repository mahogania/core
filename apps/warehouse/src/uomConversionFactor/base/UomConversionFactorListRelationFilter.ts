/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UomConversionFactorWhereInput } from "./UomConversionFactorWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class UomConversionFactorListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => UomConversionFactorWhereInput,
  })
  @ValidateNested()
  @Type(() => UomConversionFactorWhereInput)
  @IsOptional()
  @Field(() => UomConversionFactorWhereInput, {
    nullable: true,
  })
  every?: UomConversionFactorWhereInput;

  @ApiProperty({
    required: false,
    type: () => UomConversionFactorWhereInput,
  })
  @ValidateNested()
  @Type(() => UomConversionFactorWhereInput)
  @IsOptional()
  @Field(() => UomConversionFactorWhereInput, {
    nullable: true,
  })
  some?: UomConversionFactorWhereInput;

  @ApiProperty({
    required: false,
    type: () => UomConversionFactorWhereInput,
  })
  @ValidateNested()
  @Type(() => UomConversionFactorWhereInput)
  @IsOptional()
  @Field(() => UomConversionFactorWhereInput, {
    nullable: true,
  })
  none?: UomConversionFactorWhereInput;
}
export { UomConversionFactorListRelationFilter as UomConversionFactorListRelationFilter };
