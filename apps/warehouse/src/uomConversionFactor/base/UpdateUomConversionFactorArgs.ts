/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UomConversionFactorWhereUniqueInput } from "./UomConversionFactorWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UomConversionFactorUpdateInput } from "./UomConversionFactorUpdateInput";

@ArgsType()
class UpdateUomConversionFactorArgs {
  @ApiProperty({
    required: true,
    type: () => UomConversionFactorWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UomConversionFactorWhereUniqueInput)
  @Field(() => UomConversionFactorWhereUniqueInput, { nullable: false })
  where!: UomConversionFactorWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UomConversionFactorUpdateInput,
  })
  @ValidateNested()
  @Type(() => UomConversionFactorUpdateInput)
  @Field(() => UomConversionFactorUpdateInput, { nullable: false })
  data!: UomConversionFactorUpdateInput;
}

export { UpdateUomConversionFactorArgs as UpdateUomConversionFactorArgs };
