/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { StockReservationEntryWhereInput } from "./StockReservationEntryWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class StockReservationEntryListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => StockReservationEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => StockReservationEntryWhereInput)
  @IsOptional()
  @Field(() => StockReservationEntryWhereInput, {
    nullable: true,
  })
  every?: StockReservationEntryWhereInput;

  @ApiProperty({
    required: false,
    type: () => StockReservationEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => StockReservationEntryWhereInput)
  @IsOptional()
  @Field(() => StockReservationEntryWhereInput, {
    nullable: true,
  })
  some?: StockReservationEntryWhereInput;

  @ApiProperty({
    required: false,
    type: () => StockReservationEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => StockReservationEntryWhereInput)
  @IsOptional()
  @Field(() => StockReservationEntryWhereInput, {
    nullable: true,
  })
  none?: StockReservationEntryWhereInput;
}
export { StockReservationEntryListRelationFilter as StockReservationEntryListRelationFilter };
