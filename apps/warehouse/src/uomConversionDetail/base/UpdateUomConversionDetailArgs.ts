/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { UomConversionDetailWhereUniqueInput } from "./UomConversionDetailWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { UomConversionDetailUpdateInput } from "./UomConversionDetailUpdateInput";

@ArgsType()
class UpdateUomConversionDetailArgs {
  @ApiProperty({
    required: true,
    type: () => UomConversionDetailWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UomConversionDetailWhereUniqueInput)
  @Field(() => UomConversionDetailWhereUniqueInput, { nullable: false })
  where!: UomConversionDetailWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => UomConversionDetailUpdateInput,
  })
  @ValidateNested()
  @Type(() => UomConversionDetailUpdateInput)
  @Field(() => UomConversionDetailUpdateInput, { nullable: false })
  data!: UomConversionDetailUpdateInput;
}

export { UpdateUomConversionDetailArgs as UpdateUomConversionDetailArgs };
