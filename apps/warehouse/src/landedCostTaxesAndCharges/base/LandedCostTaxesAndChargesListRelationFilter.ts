/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LandedCostTaxesAndChargesWhereInput } from "./LandedCostTaxesAndChargesWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class LandedCostTaxesAndChargesListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => LandedCostTaxesAndChargesWhereInput,
  })
  @ValidateNested()
  @Type(() => LandedCostTaxesAndChargesWhereInput)
  @IsOptional()
  @Field(() => LandedCostTaxesAndChargesWhereInput, {
    nullable: true,
  })
  every?: LandedCostTaxesAndChargesWhereInput;

  @ApiProperty({
    required: false,
    type: () => LandedCostTaxesAndChargesWhereInput,
  })
  @ValidateNested()
  @Type(() => LandedCostTaxesAndChargesWhereInput)
  @IsOptional()
  @Field(() => LandedCostTaxesAndChargesWhereInput, {
    nullable: true,
  })
  some?: LandedCostTaxesAndChargesWhereInput;

  @ApiProperty({
    required: false,
    type: () => LandedCostTaxesAndChargesWhereInput,
  })
  @ValidateNested()
  @Type(() => LandedCostTaxesAndChargesWhereInput)
  @IsOptional()
  @Field(() => LandedCostTaxesAndChargesWhereInput, {
    nullable: true,
  })
  none?: LandedCostTaxesAndChargesWhereInput;
}
export { LandedCostTaxesAndChargesListRelationFilter as LandedCostTaxesAndChargesListRelationFilter };
