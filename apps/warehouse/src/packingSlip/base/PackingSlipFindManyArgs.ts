/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PackingSlipWhereInput } from "./PackingSlipWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PackingSlipOrderByInput } from "./PackingSlipOrderByInput";

@ArgsType()
class PackingSlipFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PackingSlipWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PackingSlipWhereInput, { nullable: true })
  @Type(() => PackingSlipWhereInput)
  where?: PackingSlipWhereInput;

  @ApiProperty({
    required: false,
    type: [PackingSlipOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PackingSlipOrderByInput], { nullable: true })
  @Type(() => PackingSlipOrderByInput)
  orderBy?: Array<PackingSlipOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PackingSlipFindManyArgs as PackingSlipFindManyArgs };
