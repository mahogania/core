...
const service = {
  ...
  clreDedPdlsItems: () => FIND_MANY_RESULT,
  clreDedPdls: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClreDedPdls", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClreDedPdlsService,
          useValue: service,
        },
      ],
      controllers: [ClreDedPdlsController],
      imports: [ACLModule],
    })
  ...
  test("GET /clreDedPdls/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedPdls"}/${nonExistingId}`)
  ...
  test("GET /clreDedPdls/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedPdls"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clreDedPdls existing resource", async () => {
