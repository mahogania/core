...
const service = {
  createTbClreDedMdlStsz() {
    return CREATE_RESULT;
  },
  tbClreDedMdlStszs: () => FIND_MANY_RESULT,
  tbClreDedMdlStsz: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("TbClreDedMdlStsz", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: TbClreDedMdlStszService,
          useValue: service,
        },
      ],
      controllers: [TbClreDedMdlStszController],
      imports: [ACLModule],
    })
  ...
  test("POST /tbClreDedMdlStszs", async () => {
    await request(app.getHttpServer())
      .post("/tbClreDedMdlStszs")
      .send(CREATE_INPUT)
  ...
  test("GET /tbClreDedMdlStszs", async () => {
    await request(app.getHttpServer())
      .get("/tbClreDedMdlStszs")
  ...
  test("GET /tbClreDedMdlStszs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tbClreDedMdlStszs"}/${nonExistingId}`)
  ...
  test("GET /tbClreDedMdlStszs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tbClreDedMdlStszs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /tbClreDedMdlStszs existing resource", async () => {
