...
const service = {
  ...
  clreDedMgs: () => FIND_MANY_RESULT,
  clreDedMg: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClreDedMg", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClreDedMgService,
          useValue: service,
        },
      ],
      controllers: [ClreDedMgController],
      imports: [ACLModule],
    })
  ...
  test("GET /clreDedMgs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedMgs"}/${nonExistingId}`)
  ...
  test("GET /clreDedMgs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedMgs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clreDedMgs existing resource", async () => {
