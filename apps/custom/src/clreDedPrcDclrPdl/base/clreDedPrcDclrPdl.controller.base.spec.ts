...
const service = {
  createClreDedPrcDclrPdl() {
    return CREATE_RESULT;
  },
  clreDedPrcDclrPdls: () => FIND_MANY_RESULT,
  clreDedPrcDclrPdl: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClreDedPrcDclrPdl", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClreDedPrcDclrPdlService,
          useValue: service,
        },
      ],
      controllers: [ClreDedPrcDclrPdlController],
      imports: [ACLModule],
    })
  ...
  test("POST /clreDedPrcDclrPdls", async () => {
    await request(app.getHttpServer())
      .post("/clreDedPrcDclrPdls")
      .send(CREATE_INPUT)
  ...
  test("GET /clreDedPrcDclrPdls", async () => {
    await request(app.getHttpServer())
      .get("/clreDedPrcDclrPdls")
  ...
  test("GET /clreDedPrcDclrPdls/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedPrcDclrPdls"}/${nonExistingId}`)
  ...
  test("GET /clreDedPrcDclrPdls/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedPrcDclrPdls"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clreDedPrcDclrPdls existing resource", async () => {
