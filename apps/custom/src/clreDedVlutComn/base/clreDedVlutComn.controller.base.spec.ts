...
const service = {
  createClreDedVlutComn() {
    return CREATE_RESULT;
  },
  clreDedVlutComns: () => FIND_MANY_RESULT,
  clreDedVlutComn: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClreDedVlutComn", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClreDedVlutComnService,
          useValue: service,
        },
      ],
      controllers: [ClreDedVlutComnController],
      imports: [ACLModule],
    })
  ...
  test("POST /clreDedVlutComns", async () => {
    await request(app.getHttpServer())
      .post("/clreDedVlutComns")
      .send(CREATE_INPUT)
  ...
  test("GET /clreDedVlutComns", async () => {
    await request(app.getHttpServer())
      .get("/clreDedVlutComns")
  ...
  test("GET /clreDedVlutComns/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedVlutComns"}/${nonExistingId}`)
  ...
  test("GET /clreDedVlutComns/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedVlutComns"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clreDedVlutComns existing resource", async () => {
