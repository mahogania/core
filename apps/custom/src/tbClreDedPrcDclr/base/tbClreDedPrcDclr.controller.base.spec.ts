...
const service = {
  createTbClreDedPrcDclr() {
    return CREATE_RESULT;
  },
  tbClreDedPrcDclrs: () => FIND_MANY_RESULT,
  tbClreDedPrcDclr: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("TbClreDedPrcDclr", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: TbClreDedPrcDclrService,
          useValue: service,
        },
      ],
      controllers: [TbClreDedPrcDclrController],
      imports: [ACLModule],
    })
  ...
  test("POST /tbClreDedPrcDclrs", async () => {
    await request(app.getHttpServer())
      .post("/tbClreDedPrcDclrs")
      .send(CREATE_INPUT)
  ...
  test("GET /tbClreDedPrcDclrs", async () => {
    await request(app.getHttpServer())
      .get("/tbClreDedPrcDclrs")
  ...
  test("GET /tbClreDedPrcDclrs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tbClreDedPrcDclrs"}/${nonExistingId}`)
  ...
  test("GET /tbClreDedPrcDclrs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tbClreDedPrcDclrs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /tbClreDedPrcDclrs existing resource", async () => {
