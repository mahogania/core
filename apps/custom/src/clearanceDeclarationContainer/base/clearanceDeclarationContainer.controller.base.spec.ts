...
const service = {
  ...
  clearanceDeclarationContainers: () => FIND_MANY_RESULT,
  clearanceDeclarationContainer: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClearanceDeclarationContainer", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClearanceDeclarationContainerService,
          useValue: service,
        },
      ],
      controllers: [ClearanceDeclarationContainerController],
      imports: [ACLModule],
    })
  ...
  test("POST /clearanceDeclarationContainers", async () => {
    await request(app.getHttpServer())
      .post("/clearanceDeclarationContainers")
      .send(CREATE_INPUT)
  ...
  test("GET /clearanceDeclarationContainers", async () => {
    await request(app.getHttpServer())
      .get("/clearanceDeclarationContainers")
  ...
  test("GET /clearanceDeclarationContainers/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clearanceDeclarationContainers"}/${nonExistingId}`)
  ...
  test("GET /clearanceDeclarationContainers/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clearanceDeclarationContainers"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clearanceDeclarationContainers existing resource", async () => {
