...
const service = {
  ...
  clreDedComns: () => FIND_MANY_RESULT,
  clreDedComn: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ClreDedComn", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: ClreDedComnService,
          useValue: service,
        },
      ],
      controllers: [ClreDedComnController],
      imports: [ACLModule],
    })
  ...
  test("GET /clreDedComns/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedComns"}/${nonExistingId}`)
  ...
  test("GET /clreDedComns/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/clreDedComns"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /clreDedComns existing resource", async () => {
