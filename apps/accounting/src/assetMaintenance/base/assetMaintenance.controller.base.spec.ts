...
const CREATE_INPUT = {
  ...
  maintenanceTeam: "exampleMaintenanceTeam",
  modified: new Date(),
...
const service = {
  ...
  assetMaintenances: () => FIND_MANY_RESULT,
  assetMaintenance: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("AssetMaintenance", () => {
  ...
  test("POST /assetMaintenances", async () => {
    await request(app.getHttpServer())
      .post("/assetMaintenances")
      .send(CREATE_INPUT)
  ...
  test("GET /assetMaintenances", async () => {
    await request(app.getHttpServer())
  ...
  test("GET /assetMaintenances/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/assetMaintenances"}/${nonExistingId}`)
  ...
  test("GET /assetMaintenances/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/assetMaintenances"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /assetMaintenances existing resource", async () => {
