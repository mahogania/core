/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AssetMaintenanceWhereUniqueInput } from "./AssetMaintenanceWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { AssetMaintenanceUpdateInput } from "./AssetMaintenanceUpdateInput";

@ArgsType()
class UpdateAssetMaintenanceArgs {
  @ApiProperty({
    required: true,
    type: () => AssetMaintenanceWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => AssetMaintenanceWhereUniqueInput)
  @Field(() => AssetMaintenanceWhereUniqueInput, { nullable: false })
  where!: AssetMaintenanceWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => AssetMaintenanceUpdateInput,
  })
  @ValidateNested()
  @Type(() => AssetMaintenanceUpdateInput)
  @Field(() => AssetMaintenanceUpdateInput, { nullable: false })
  data!: AssetMaintenanceUpdateInput;
}

export { UpdateAssetMaintenanceArgs as UpdateAssetMaintenanceArgs };
