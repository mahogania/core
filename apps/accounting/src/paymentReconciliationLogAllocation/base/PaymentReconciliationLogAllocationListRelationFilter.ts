/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PaymentReconciliationLogAllocationWhereInput } from "./PaymentReconciliationLogAllocationWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class PaymentReconciliationLogAllocationListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => PaymentReconciliationLogAllocationWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentReconciliationLogAllocationWhereInput)
  @IsOptional()
  @Field(() => PaymentReconciliationLogAllocationWhereInput, {
    nullable: true,
  })
  every?: PaymentReconciliationLogAllocationWhereInput;

  @ApiProperty({
    required: false,
    type: () => PaymentReconciliationLogAllocationWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentReconciliationLogAllocationWhereInput)
  @IsOptional()
  @Field(() => PaymentReconciliationLogAllocationWhereInput, {
    nullable: true,
  })
  some?: PaymentReconciliationLogAllocationWhereInput;

  @ApiProperty({
    required: false,
    type: () => PaymentReconciliationLogAllocationWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentReconciliationLogAllocationWhereInput)
  @IsOptional()
  @Field(() => PaymentReconciliationLogAllocationWhereInput, {
    nullable: true,
  })
  none?: PaymentReconciliationLogAllocationWhereInput;
}
export { PaymentReconciliationLogAllocationListRelationFilter as PaymentReconciliationLogAllocationListRelationFilter };
