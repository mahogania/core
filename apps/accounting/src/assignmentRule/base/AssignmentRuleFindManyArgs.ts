/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AssignmentRuleWhereInput } from "./AssignmentRuleWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AssignmentRuleOrderByInput } from "./AssignmentRuleOrderByInput";

@ArgsType()
class AssignmentRuleFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AssignmentRuleWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AssignmentRuleWhereInput, { nullable: true })
  @Type(() => AssignmentRuleWhereInput)
  where?: AssignmentRuleWhereInput;

  @ApiProperty({
    required: false,
    type: [AssignmentRuleOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AssignmentRuleOrderByInput], { nullable: true })
  @Type(() => AssignmentRuleOrderByInput)
  orderBy?: Array<AssignmentRuleOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AssignmentRuleFindManyArgs as AssignmentRuleFindManyArgs };
