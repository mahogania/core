/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { BankAccountSubtypeWhereInput } from "./BankAccountSubtypeWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { BankAccountSubtypeOrderByInput } from "./BankAccountSubtypeOrderByInput";

@ArgsType()
class BankAccountSubtypeFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => BankAccountSubtypeWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => BankAccountSubtypeWhereInput, { nullable: true })
  @Type(() => BankAccountSubtypeWhereInput)
  where?: BankAccountSubtypeWhereInput;

  @ApiProperty({
    required: false,
    type: [BankAccountSubtypeOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [BankAccountSubtypeOrderByInput], { nullable: true })
  @Type(() => BankAccountSubtypeOrderByInput)
  orderBy?: Array<BankAccountSubtypeOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { BankAccountSubtypeFindManyArgs as BankAccountSubtypeFindManyArgs };
