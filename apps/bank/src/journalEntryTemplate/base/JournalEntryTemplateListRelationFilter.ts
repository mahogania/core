/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { JournalEntryTemplateWhereInput } from "./JournalEntryTemplateWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class JournalEntryTemplateListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => JournalEntryTemplateWhereInput,
  })
  @ValidateNested()
  @Type(() => JournalEntryTemplateWhereInput)
  @IsOptional()
  @Field(() => JournalEntryTemplateWhereInput, {
    nullable: true,
  })
  every?: JournalEntryTemplateWhereInput;

  @ApiProperty({
    required: false,
    type: () => JournalEntryTemplateWhereInput,
  })
  @ValidateNested()
  @Type(() => JournalEntryTemplateWhereInput)
  @IsOptional()
  @Field(() => JournalEntryTemplateWhereInput, {
    nullable: true,
  })
  some?: JournalEntryTemplateWhereInput;

  @ApiProperty({
    required: false,
    type: () => JournalEntryTemplateWhereInput,
  })
  @ValidateNested()
  @Type(() => JournalEntryTemplateWhereInput)
  @IsOptional()
  @Field(() => JournalEntryTemplateWhereInput, {
    nullable: true,
  })
  none?: JournalEntryTemplateWhereInput;
}
export { JournalEntryTemplateListRelationFilter as JournalEntryTemplateListRelationFilter };
