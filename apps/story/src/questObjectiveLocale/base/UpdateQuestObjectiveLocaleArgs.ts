/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestObjectiveLocaleWhereUniqueInput } from "./QuestObjectiveLocaleWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { QuestObjectiveLocaleUpdateInput } from "./QuestObjectiveLocaleUpdateInput";

@ArgsType()
class UpdateQuestObjectiveLocaleArgs {
  @ApiProperty({
    required: true,
    type: () => QuestObjectiveLocaleWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QuestObjectiveLocaleWhereUniqueInput)
  @Field(() => QuestObjectiveLocaleWhereUniqueInput, { nullable: false })
  where!: QuestObjectiveLocaleWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => QuestObjectiveLocaleUpdateInput,
  })
  @ValidateNested()
  @Type(() => QuestObjectiveLocaleUpdateInput)
  @Field(() => QuestObjectiveLocaleUpdateInput, { nullable: false })
  data!: QuestObjectiveLocaleUpdateInput;
}

export { UpdateQuestObjectiveLocaleArgs as UpdateQuestObjectiveLocaleArgs };
