/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestGreetingLocaleWhereUniqueInput } from "./QuestGreetingLocaleWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { QuestGreetingLocaleUpdateInput } from "./QuestGreetingLocaleUpdateInput";

@ArgsType()
class UpdateQuestGreetingLocaleArgs {
  @ApiProperty({
    required: true,
    type: () => QuestGreetingLocaleWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QuestGreetingLocaleWhereUniqueInput)
  @Field(() => QuestGreetingLocaleWhereUniqueInput, { nullable: false })
  where!: QuestGreetingLocaleWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => QuestGreetingLocaleUpdateInput,
  })
  @ValidateNested()
  @Type(() => QuestGreetingLocaleUpdateInput)
  @Field(() => QuestGreetingLocaleUpdateInput, { nullable: false })
  data!: QuestGreetingLocaleUpdateInput;
}

export { UpdateQuestGreetingLocaleArgs as UpdateQuestGreetingLocaleArgs };
