/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestRequestItemsLocaleWhereInput } from "./QuestRequestItemsLocaleWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class QuestRequestItemsLocaleListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => QuestRequestItemsLocaleWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRequestItemsLocaleWhereInput)
  @IsOptional()
  @Field(() => QuestRequestItemsLocaleWhereInput, {
    nullable: true,
  })
  every?: QuestRequestItemsLocaleWhereInput;

  @ApiProperty({
    required: false,
    type: () => QuestRequestItemsLocaleWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRequestItemsLocaleWhereInput)
  @IsOptional()
  @Field(() => QuestRequestItemsLocaleWhereInput, {
    nullable: true,
  })
  some?: QuestRequestItemsLocaleWhereInput;

  @ApiProperty({
    required: false,
    type: () => QuestRequestItemsLocaleWhereInput,
  })
  @ValidateNested()
  @Type(() => QuestRequestItemsLocaleWhereInput)
  @IsOptional()
  @Field(() => QuestRequestItemsLocaleWhereInput, {
    nullable: true,
  })
  none?: QuestRequestItemsLocaleWhereInput;
}
export { QuestRequestItemsLocaleListRelationFilter as QuestRequestItemsLocaleListRelationFilter };
