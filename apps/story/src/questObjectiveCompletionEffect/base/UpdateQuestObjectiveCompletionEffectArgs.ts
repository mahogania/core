/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QuestObjectiveCompletionEffectWhereUniqueInput } from "./QuestObjectiveCompletionEffectWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { QuestObjectiveCompletionEffectUpdateInput } from "./QuestObjectiveCompletionEffectUpdateInput";

@ArgsType()
class UpdateQuestObjectiveCompletionEffectArgs {
  @ApiProperty({
    required: true,
    type: () => QuestObjectiveCompletionEffectWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QuestObjectiveCompletionEffectWhereUniqueInput)
  @Field(() => QuestObjectiveCompletionEffectWhereUniqueInput, {
    nullable: false,
  })
  where!: QuestObjectiveCompletionEffectWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => QuestObjectiveCompletionEffectUpdateInput,
  })
  @ValidateNested()
  @Type(() => QuestObjectiveCompletionEffectUpdateInput)
  @Field(() => QuestObjectiveCompletionEffectUpdateInput, { nullable: false })
  data!: QuestObjectiveCompletionEffectUpdateInput;
}

export { UpdateQuestObjectiveCompletionEffectArgs as UpdateQuestObjectiveCompletionEffectArgs };
