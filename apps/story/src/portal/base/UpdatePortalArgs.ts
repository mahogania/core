/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PortalWhereUniqueInput } from "./PortalWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { PortalUpdateInput } from "./PortalUpdateInput";

@ArgsType()
class UpdatePortalArgs {
  @ApiProperty({
    required: true,
    type: () => PortalWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => PortalWhereUniqueInput)
  @Field(() => PortalWhereUniqueInput, { nullable: false })
  where!: PortalWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => PortalUpdateInput,
  })
  @ValidateNested()
  @Type(() => PortalUpdateInput)
  @Field(() => PortalUpdateInput, { nullable: false })
  data!: PortalUpdateInput;
}

export { UpdatePortalArgs as UpdatePortalArgs };
