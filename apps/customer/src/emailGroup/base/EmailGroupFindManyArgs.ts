/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EmailGroupWhereInput } from "./EmailGroupWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EmailGroupOrderByInput } from "./EmailGroupOrderByInput";

@ArgsType()
class EmailGroupFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EmailGroupWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EmailGroupWhereInput, { nullable: true })
  @Type(() => EmailGroupWhereInput)
  where?: EmailGroupWhereInput;

  @ApiProperty({
    required: false,
    type: [EmailGroupOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EmailGroupOrderByInput], { nullable: true })
  @Type(() => EmailGroupOrderByInput)
  orderBy?: Array<EmailGroupOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EmailGroupFindManyArgs as EmailGroupFindManyArgs };
