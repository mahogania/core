/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EmailCampaignWhereUniqueInput } from "./EmailCampaignWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { EmailCampaignUpdateInput } from "./EmailCampaignUpdateInput";

@ArgsType()
class UpdateEmailCampaignArgs {
  @ApiProperty({
    required: true,
    type: () => EmailCampaignWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EmailCampaignWhereUniqueInput)
  @Field(() => EmailCampaignWhereUniqueInput, { nullable: false })
  where!: EmailCampaignWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => EmailCampaignUpdateInput,
  })
  @ValidateNested()
  @Type(() => EmailCampaignUpdateInput)
  @Field(() => EmailCampaignUpdateInput, { nullable: false })
  data!: EmailCampaignUpdateInput;
}

export { UpdateEmailCampaignArgs as UpdateEmailCampaignArgs };
