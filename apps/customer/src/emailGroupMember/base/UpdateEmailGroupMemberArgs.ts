/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EmailGroupMemberWhereUniqueInput } from "./EmailGroupMemberWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { EmailGroupMemberUpdateInput } from "./EmailGroupMemberUpdateInput";

@ArgsType()
class UpdateEmailGroupMemberArgs {
  @ApiProperty({
    required: true,
    type: () => EmailGroupMemberWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EmailGroupMemberWhereUniqueInput)
  @Field(() => EmailGroupMemberWhereUniqueInput, { nullable: false })
  where!: EmailGroupMemberWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => EmailGroupMemberUpdateInput,
  })
  @ValidateNested()
  @Type(() => EmailGroupMemberUpdateInput)
  @Field(() => EmailGroupMemberUpdateInput, { nullable: false })
  data!: EmailGroupMemberUpdateInput;
}

export { UpdateEmailGroupMemberArgs as UpdateEmailGroupMemberArgs };
