/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CreatureImmunityWhereUniqueInput } from "./CreatureImmunityWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CreatureImmunityUpdateInput } from "./CreatureImmunityUpdateInput";

@ArgsType()
class UpdateCreatureImmunityArgs {
  @ApiProperty({
    required: true,
    type: () => CreatureImmunityWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CreatureImmunityWhereUniqueInput)
  @Field(() => CreatureImmunityWhereUniqueInput, { nullable: false })
  where!: CreatureImmunityWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CreatureImmunityUpdateInput,
  })
  @ValidateNested()
  @Type(() => CreatureImmunityUpdateInput)
  @Field(() => CreatureImmunityUpdateInput, { nullable: false })
  data!: CreatureImmunityUpdateInput;
}

export { UpdateCreatureImmunityArgs as UpdateCreatureImmunityArgs };
