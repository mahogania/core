...
const service = {
  createCreatureFormation() {
    return CREATE_RESULT;
  },
  creatureFormations: () => FIND_MANY_RESULT,
  creatureFormation: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("CreatureFormation", () => {
  ...
  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      providers: [
        {
          provide: CreatureFormationService,
          useValue: service,
        },
      ],
      controllers: [CreatureFormationController],
      imports: [ACLModule],
    })
