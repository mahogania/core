/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ConversationActorsWhereInput } from "./ConversationActorsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ConversationActorsOrderByInput } from "./ConversationActorsOrderByInput";

@ArgsType()
class ConversationActorsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ConversationActorsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ConversationActorsWhereInput, { nullable: true })
  @Type(() => ConversationActorsWhereInput)
  where?: ConversationActorsWhereInput;

  @ApiProperty({
    required: false,
    type: [ConversationActorsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ConversationActorsOrderByInput], { nullable: true })
  @Type(() => ConversationActorsOrderByInput)
  orderBy?: Array<ConversationActorsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ConversationActorsFindManyArgs as ConversationActorsFindManyArgs };
