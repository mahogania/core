/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CreatureLevelStatWhereInput } from "./CreatureLevelStatWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CreatureLevelStatOrderByInput } from "./CreatureLevelStatOrderByInput";

@ArgsType()
class CreatureLevelStatFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CreatureLevelStatWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CreatureLevelStatWhereInput, { nullable: true })
  @Type(() => CreatureLevelStatWhereInput)
  where?: CreatureLevelStatWhereInput;

  @ApiProperty({
    required: false,
    type: [CreatureLevelStatOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CreatureLevelStatOrderByInput], { nullable: true })
  @Type(() => CreatureLevelStatOrderByInput)
  orderBy?: Array<CreatureLevelStatOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CreatureLevelStatFindManyArgs as CreatureLevelStatFindManyArgs };
