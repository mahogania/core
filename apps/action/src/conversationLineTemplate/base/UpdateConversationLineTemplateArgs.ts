/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ConversationLineTemplateWhereUniqueInput } from "./ConversationLineTemplateWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ConversationLineTemplateUpdateInput } from "./ConversationLineTemplateUpdateInput";

@ArgsType()
class UpdateConversationLineTemplateArgs {
  @ApiProperty({
    required: true,
    type: () => ConversationLineTemplateWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ConversationLineTemplateWhereUniqueInput)
  @Field(() => ConversationLineTemplateWhereUniqueInput, { nullable: false })
  where!: ConversationLineTemplateWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ConversationLineTemplateUpdateInput,
  })
  @ValidateNested()
  @Type(() => ConversationLineTemplateUpdateInput)
  @Field(() => ConversationLineTemplateUpdateInput, { nullable: false })
  data!: ConversationLineTemplateUpdateInput;
}

export { UpdateConversationLineTemplateArgs as UpdateConversationLineTemplateArgs };
