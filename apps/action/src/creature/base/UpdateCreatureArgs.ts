/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CreatureWhereUniqueInput } from "./CreatureWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CreatureUpdateInput } from "./CreatureUpdateInput";

@ArgsType()
class UpdateCreatureArgs {
  @ApiProperty({
    required: true,
    type: () => CreatureWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CreatureWhereUniqueInput)
  @Field(() => CreatureWhereUniqueInput, { nullable: false })
  where!: CreatureWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CreatureUpdateInput,
  })
  @ValidateNested()
  @Type(() => CreatureUpdateInput)
  @Field(() => CreatureUpdateInput, { nullable: false })
  data!: CreatureUpdateInput;
}

export { UpdateCreatureArgs as UpdateCreatureArgs };
