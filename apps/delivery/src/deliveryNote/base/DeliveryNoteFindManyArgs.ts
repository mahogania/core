/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DeliveryNoteWhereInput } from "./DeliveryNoteWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DeliveryNoteOrderByInput } from "./DeliveryNoteOrderByInput";

@ArgsType()
class DeliveryNoteFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DeliveryNoteWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DeliveryNoteWhereInput, { nullable: true })
  @Type(() => DeliveryNoteWhereInput)
  where?: DeliveryNoteWhereInput;

  @ApiProperty({
    required: false,
    type: [DeliveryNoteOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DeliveryNoteOrderByInput], { nullable: true })
  @Type(() => DeliveryNoteOrderByInput)
  orderBy?: Array<DeliveryNoteOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DeliveryNoteFindManyArgs as DeliveryNoteFindManyArgs };
