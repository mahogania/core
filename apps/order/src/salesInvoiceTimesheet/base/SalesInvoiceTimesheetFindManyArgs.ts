/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SalesInvoiceTimesheetWhereInput } from "./SalesInvoiceTimesheetWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SalesInvoiceTimesheetOrderByInput } from "./SalesInvoiceTimesheetOrderByInput";

@ArgsType()
class SalesInvoiceTimesheetFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SalesInvoiceTimesheetWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SalesInvoiceTimesheetWhereInput, { nullable: true })
  @Type(() => SalesInvoiceTimesheetWhereInput)
  where?: SalesInvoiceTimesheetWhereInput;

  @ApiProperty({
    required: false,
    type: [SalesInvoiceTimesheetOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SalesInvoiceTimesheetOrderByInput], { nullable: true })
  @Type(() => SalesInvoiceTimesheetOrderByInput)
  orderBy?: Array<SalesInvoiceTimesheetOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SalesInvoiceTimesheetFindManyArgs as SalesInvoiceTimesheetFindManyArgs };
