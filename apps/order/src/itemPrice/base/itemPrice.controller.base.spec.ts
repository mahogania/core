...
const service = {
  ...
  itemPrices: () => FIND_MANY_RESULT,
  itemPrice: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("ItemPrice", () => {
  ...
  test("GET /itemPrices/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/itemPrices"}/${nonExistingId}`)
  ...
  test("GET /itemPrices/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/itemPrices"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /itemPrices existing resource", async () => {
