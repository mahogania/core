/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PurchaseInvoiceItemWhereInput } from "./PurchaseInvoiceItemWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PurchaseInvoiceItemOrderByInput } from "./PurchaseInvoiceItemOrderByInput";

@ArgsType()
class PurchaseInvoiceItemFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PurchaseInvoiceItemWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PurchaseInvoiceItemWhereInput, { nullable: true })
  @Type(() => PurchaseInvoiceItemWhereInput)
  where?: PurchaseInvoiceItemWhereInput;

  @ApiProperty({
    required: false,
    type: [PurchaseInvoiceItemOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PurchaseInvoiceItemOrderByInput], { nullable: true })
  @Type(() => PurchaseInvoiceItemOrderByInput)
  orderBy?: Array<PurchaseInvoiceItemOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PurchaseInvoiceItemFindManyArgs as PurchaseInvoiceItemFindManyArgs };
