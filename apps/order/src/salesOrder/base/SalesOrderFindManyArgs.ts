/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SalesOrderWhereInput } from "./SalesOrderWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SalesOrderOrderByInput } from "./SalesOrderOrderByInput";

@ArgsType()
class SalesOrderFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SalesOrderWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SalesOrderWhereInput, { nullable: true })
  @Type(() => SalesOrderWhereInput)
  where?: SalesOrderWhereInput;

  @ApiProperty({
    required: false,
    type: [SalesOrderOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SalesOrderOrderByInput], { nullable: true })
  @Type(() => SalesOrderOrderByInput)
  orderBy?: Array<SalesOrderOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SalesOrderFindManyArgs as SalesOrderFindManyArgs };
