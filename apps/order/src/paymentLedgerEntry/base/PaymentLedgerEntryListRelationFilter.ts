/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PaymentLedgerEntryWhereInput } from "./PaymentLedgerEntryWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class PaymentLedgerEntryListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => PaymentLedgerEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentLedgerEntryWhereInput)
  @IsOptional()
  @Field(() => PaymentLedgerEntryWhereInput, {
    nullable: true,
  })
  every?: PaymentLedgerEntryWhereInput;

  @ApiProperty({
    required: false,
    type: () => PaymentLedgerEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentLedgerEntryWhereInput)
  @IsOptional()
  @Field(() => PaymentLedgerEntryWhereInput, {
    nullable: true,
  })
  some?: PaymentLedgerEntryWhereInput;

  @ApiProperty({
    required: false,
    type: () => PaymentLedgerEntryWhereInput,
  })
  @ValidateNested()
  @Type(() => PaymentLedgerEntryWhereInput)
  @IsOptional()
  @Field(() => PaymentLedgerEntryWhereInput, {
    nullable: true,
  })
  none?: PaymentLedgerEntryWhereInput;
}
export { PaymentLedgerEntryListRelationFilter as PaymentLedgerEntryListRelationFilter };
