/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { WeaknessWhereInput } from "./WeaknessWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { WeaknessOrderByInput } from "./WeaknessOrderByInput";

@ArgsType()
class WeaknessFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => WeaknessWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => WeaknessWhereInput, { nullable: true })
  @Type(() => WeaknessWhereInput)
  where?: WeaknessWhereInput;

  @ApiProperty({
    required: false,
    type: [WeaknessOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [WeaknessOrderByInput], { nullable: true })
  @Type(() => WeaknessOrderByInput)
  orderBy?: Array<WeaknessOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { WeaknessFindManyArgs as WeaknessFindManyArgs };
